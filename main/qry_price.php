...
if(isset($attributes['pricelist_id']) and isset($attributes['artikul']) and $attributes['act'] !='add_cart'){
	$query = "SELECT p.id,p.str_code1,p.str_barcode,CONCAT(g.id,'.',g.extention) AS img,p.str_name,p.str_state,p.str_volume,p.str_package,p.num_price_single,p.num_price_pack,p.num_amount, p.expiration, p.pricelist_id
                FROM pricelist AS p, goods_pic AS g 

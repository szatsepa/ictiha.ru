...
$rgb=0xffffff; //цвет заливки несоответстви€
$size = getimagesize($source);//узнаем размеры картинки (дает нам масив size)
$format = strtolower(substr($size['mime'], strpos($size['mime'], '/')+1)); //определ€ем тип файла
...
$x_ratio = $width / $size[0]; //пропорци€ ширины будущего превью
$y_ratio = $height / $size[1]; //пропорци€ высоты будущего превью
...
imagefill($img, 0, 0, $rgb); //заливаем егоЕ
